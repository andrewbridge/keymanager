 ...
 *				 browsers.
 *
 * @param eventDefinitions An object of objects, containing definitions of the custom key events that
 *                         should be fired. Each definition should have the following signature.
 ...
 */
function KeyManager(evtDefs) {
	this.listeners = {keydown: this._listener.bind(this), keyup: this._updateStateInfo.bind(this)};
    document.addEventListener("keydown", this.listeners.keydown, true);
    document.addEventListener("keyup", this.listeners.keyup, true);
    this.eventDefinitions = evtDefs;
    this.keysPressed = [];
};
 ...
 * listener
 * 
 * When fired, listener checks whether the key has an entry in the eventDefinition object
 ...
 */
KeyManager.prototype._listener = function(e) {
    this._updateStateInfo(e);
 ...
 * _updateStateInfo
 *
 * Updates the keysPressed array when a key is pressed or released.
 * 
 * @param e An event object, the method will only react to keyup or keydown events.
*/
KeyManager.prototype._updateStateInfo = function(e) {
 ...
 * isPressed
 * 
 * Determines whether or not a key is currently pressed or not.
 * 
...
*/
KeyManager.prototype.isPressed = function(keyCode) {
    keyCode = (typeof keyCode == "string" && keyCode in this._keyMappings) ? this._keyMappings[keyCode] : keyCode;
 ...
 * changeTabNav
 * 
 * Changes the element(s) place in the tab order.
...
*/
KeyManager.prototype.changeTabNav = function(elms, val) {
    var result = false;
...
* keyMappings
*
* The object which contains key names to key codes.
*/
KeyManager.prototype._keyMappings = {
